...
system). Leads because it is current, owned, and more than a website.

**2. Bon House** (current)
Discipline: engineering + design. Showable: yes, but check the Bon agreements first (see
confidentiality below). Angle: web presence for an Atlanta venue gaining momentum. Design-forward,
on-brand for the aesthetic, live and growing.

**3. Georgia Tech curriculum engineering** (ready to describe)
Discipline: engineering + education. Showable: describe and credit; confirm which curriculum
material is shareable. Angle: six years teaching the full-stack flex program (lead instructor,
15-plus students at a time) plus a stint as curriculum engineer on a full overhaul that added AI
and LangChain, TypeScript, CI/CD, TDD tooling, and a Python module. Reframes teaching as
building learning systems at scale and turns the IC-narrative friction into an "I architect
systems and direct teams" story.

**4. Horton Group platform** (ready to describe)
Discipline: engineering. Showable: describe; client assets likely not showable. Angle: built a
reusable Angular 4 / KeystoneJS templated application with their senior developer, designed so
Horton could re-skin it for future clients, plus a remediation win taking a bloated WordPress
site (Sisters on the Fly) back to a maintainable state. Architecting for reuse and remediation
are both senior signals.
...
## Tier 2: Experience and credits (closing-page list, names not stories)

Grouped and listed as credits. State the contribution precisely; show only cleared assets.
...
**Film credits:**
- 1st AD, short film (Wilmington, NC).
- Production assistant, multiple shorts and one feature.

**Game jams (if not pulled into the featured games study):**
- Is Anyone Out There?, La Bamba.
...
## Confirmations needed (quick)

- "Four Tops": the restaurant group, or four top-tier restaurants?
- Which three NSS capstones, and the order you want to refresh them.
- Which of your own films is the flagship for the hero or lab, if any.
- Whether Punchy Fox plus the jams ship as one combined "games" case study or as the lead-in to
  the lab.
